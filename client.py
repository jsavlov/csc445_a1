...
print "Latency 1 byte result: " + str(float("%.5f" % (lat1_rtt_time * 1000)))
...
print "Latency 32 byte result: " + str(float("%.5f" % (lat32_rtt_time * 1000)))
...
print "Latency 1024 byte result: " + str(float("%.5f" % (lat1024_rtt_time * 1000)))

host_socket = fresh_client_socket(HOST_SERVER, PORT, using_udp)
print "Throughput test: 1 KBytes"
tp1k_tx_rate = calc_throughput(1000, host_socket)
print "Throughput 1 Kbyte result: " + str(float("%.5f" % (tp1k_tx_rate / 1000000)))

host_socket = fresh_client_socket(HOST_SERVER, PORT, using_udp)
print "Throughput test: 16 KBytes"
tp16k_tx_rate = calc_throughput(16000, host_socket)
print "Throughput 16 Kbyte result: " + str(float("%.5f" % (tp16k_tx_rate / 1000000)))

host_socket = fresh_client_socket(HOST_SERVER, PORT, using_udp)
print "Throughput test: 64 KBytes"
tp64k_tx_rate = calc_throughput(64000, host_socket)
print "Throughput 64 Kbyte result: " + str(float("%.5f" % (tp64k_tx_rate / 1000000)))

host_socket = fresh_client_socket(HOST_SERVER, PORT, using_udp)
print "Throughput test: 256 KBytes"
tp256k_tx_rate = calc_throughput(256000, host_socket)
print "Throughput 256 Kbyte result: " + str(float("%.5f" % (tp256k_tx_rate / 1000000)))

host_socket = fresh_client_socket(HOST_SERVER, PORT, using_udp)
print "Throughput test: 1 MBytes"
tp1m_tx_rate = calc_throughput(1000000, host_socket)
print "Throughput 1 Mbyte result: " + str(float("%.5f" % (tp1m_tx_rate / 1000000)))
...
if using_udp is False:
    ...
    print "Interaction test 256 x 4KByte result: " + str(float("%.5f" % interaction_4kb_time))

    host_socket = fresh_client_socket(HOST_SERVER, PORT, using_udp)
    print "Interaction test: 512 x 2KByte messages"
    interaction_2kb_time = observe_interaction(2048, 512, host_socket)
    print "Interaction test 512 x 2KByte result: " + str(float("%.5f" % interaction_2kb_time))

    host_socket = fresh_client_socket(HOST_SERVER, PORT, using_udp)
    print "Interaction test: 1024 x 1KByte messages"
    interaction_1kb_time = observe_interaction(1024, 1024, host_socket)
    print "Interaction test 1024 x 1KByte result: " + str(float("%.5f" % interaction_1kb_time))
